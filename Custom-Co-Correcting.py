...
class CoCorrecting(BasicTrainer, Loss):
    ...
    def _compute_loss(self, outputA, outputB, target, target_var, index, epoch, i, parallel=False):
        ...
        # Final fine-tuning based on corrected labels
            ...
            if self.args.forget_type == 'coteaching':
                ...
                pure_ratio_discard_1, pure_ratio_discard_2 = self.loss_coteaching(outputA, outputB,
    ...
    def training(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            print('-----------------')
            if epoch < self.args.warmup:
                print(f"[Epoch {epoch}] Mode: Warmup (Simple CE, No Selection)")
            elif epoch < self.args.stage1:
                print(f"[Epoch {epoch}] Mode: Pre-Stage 1 (Hard Label Co-teaching(plus))")
            elif epoch < self.args.stage2:
                print(f"[Epoch {epoch}] Mode: Stage 1 (PENCIL Soft Label Update)")
            else:
                print(f"[Epoch {epoch}] Mode: Stage 2+ (PENCIL KL Finetune)")
            
            # unfreeze
            # Only unfreeze if using pretrained recipe + warmup
    ...
    def train(self, epoch=0):
        ...
        # Extra Metrics for Train (using Mix results for 3, and shared disagree)
